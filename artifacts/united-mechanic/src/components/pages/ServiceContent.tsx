...
import { Phone, CheckCircle2, ChevronDown, ChevronUp, ArrowRight } from "lucide-react";
...
export default function ServiceContent({ service }: { service: ServiceData }) {
  ...
  const heroStats = service.category === "body-shop"
    ? [{ label: "Insurance", sub: "All Major Carriers" },{ label: "Color", sub: "Computer Matched" },{ label: "Warranty", sub: "Parts & Labor" }]
    : [{ label: "Same-Day", sub: "Service Available" },{ label: "All Makes", sub: "& Models" },{ label: "Warranty", sub: "Parts & Labor" }];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-14">
              {service.sections.map((section, i) => (
                <motion.section key={section.heading} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.05 }} aria-labelledby={`section-heading-${i}`}>
                  <h2 id={`section-heading-${i}`} className="text-xl md:text-2xl font-bold text-foreground mb-3">{section.heading}</h2>
                  <p className="text-muted-foreground text-base leading-relaxed">{section.body}</p>
                </motion.section>
              ))}

              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} aria-labelledby="included-heading">
                <h2 id="included-heading" className="text-2xl font-bold text-foreground mb-5">What's Included in Our {service.title}</h2>
              ...
              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} aria-labelledby="process-heading">
                <h2 id="process-heading" className="text-2xl font-bold text-foreground mb-6">Our {service.title} Process</h2>
                <div className="space-y-4">
                  {service.process.map((step, i) => (
                    <div key={step.step} className="flex gap-4">
                      <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary text-white text-sm font-bold flex items-center justify-center mt-0.5">{i + 1}</div>
                      <div>
              ...
              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }} aria-labelledby="why-heading">
                <h2 id="why-heading" className="text-2xl font-bold text-foreground mb-5">Why {locationKeyword} Drivers Choose My Ride Service Center</h2>
              ...
              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} aria-labelledby="faq-heading">
                <h2 id="faq-heading" className="text-2xl font-bold text-foreground mb-6">Frequently Asked Questions About {service.title}</h2>
                <div className="space-y-3">{service.faq.map(({ q, a }) => <FAQItem key={q} q={q} a={a} />)}</div>
              </motion.section>

              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} className="bg-zinc-950 rounded-3xl p-8 text-center">
                <p className="text-xs font-bold uppercase tracking-widest text-primary mb-3">My Ride Service Center</p>
                <h2 className="text-2xl font-bold text-white mb-2">Ready to Schedule Your {service.title}?</h2>
            ...
            <motion.aside initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-5">
              <div className="bg-zinc-950 rounded-3xl border border-zinc-800 overflow-hidden sticky top-24">
                ...
                <div className="px-6 pb-6 border-t border-zinc-800 pt-5">
                  ...
                  <p className="text-xs text-zinc-500 leading-relaxed">Proudly serving Fredericksburg, Spotsylvania, Stafford, King George, and Culpeper counties.</p>
                </div>
              </div>
            </motion.aside>
          </div>
      ...
      <section className="bg-zinc-50 border-t border-border py-16">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href={backHref}>
              <Button variant="outline" size="sm" className="font-semibold gap-1.5 hidden sm:flex">View All <ArrowRight className="h-3.5 w-3.5" /></Button>
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {relatedServices.map((s, i) => (
              <motion.div key={s.slug} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.07 }}>
                <Link href={`${backHref}/${s.slug}`} className="group block h-full">
                  <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm hover:shadow-md transition-shadow h-full flex flex-col">
                    <div className="relative h-44 overflow-hidden">
                      <img src={s.image} alt={`${s.title} in Fredericksburg, VA`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                    </div>
                    <div className="p-5 flex flex-col flex-1">
                      <h3 className="font-bold text-foreground text-base mb-1.5 group-hover:text-primary transition-colors">{s.title}</h3>
                      <p className="text-sm text-muted-foreground leading-relaxed flex-1">{s.tagline}</p>
                      <div className="flex items-center gap-1 mt-4 text-primary text-sm font-semibold">Learn More <ArrowRight className="h-3.5 w-3.5" /></div>
                    </div>
                  </div>
                </Link>
              </motion.div>
          ...
          <div className="mt-6 sm:hidden text-center">
            <Link href={backHref}><Button variant="outline" className="font-semibold gap-1.5">View All {backLabel} Services <ArrowRight className="h-3.5 w-3.5" /></Button></Link>
          </div>
        </div>
      </section>
    </div>
