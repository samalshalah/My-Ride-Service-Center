        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="lg:col-span-2">
              <form className="bg-white rounded-3xl border border-border shadow-sm p-8 space-y-6">
                <div>
                ...
                <div className="border-t border-border pt-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <Select onValueChange={(v) => set("service", v)}>
                        <SelectTrigger><SelectValue placeholder="Select a service" /></SelectTrigger>
                        <SelectContent>
                          {ALL_SERVICE_OPTIONS.map(({ group, services }) => (
                            <React.Fragment key={group}>
                              <SelectItem value={`__group_${group}`} disabled className="font-semibold text-foreground">— {group} —</SelectItem>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-6">
              <div className="bg-white rounded-3xl border border-border shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4"><Clock className="h-5 w-5 text-primary" /><h3 className="font-bold text-foreground">Hours of Operation</h3></div>
                <ul className="space-y-2 text-sm">
                  {[{ day: "Monday – Friday", hours: "8:00 AM – 6:00 PM" },{ day: "Saturday", hours: "9:00 AM – 3:00 PM" },{ day: "Sunday", hours: "Closed" }].map(({ day, hours }) => (
              ...
              <div className="bg-white rounded-3xl border border-border shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4"><MapPin className="h-5 w-5 text-primary" /><h3 className="font-bold text-foreground">Our Location</h3></div>
              ...
              <div className="bg-white rounded-3xl border border-border shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4"><Car className="h-5 w-5 text-primary" /><h3 className="font-bold text-foreground">What to Expect</h3></div>
                <ul className="space-y-3">
                  {["Our team will call to confirm your appointment","Our technicians will inspect and diagnose","You'll get a clear estimate before any work begins","All repairs backed by our quality guarantee"].map((item) => (
              ...
              <div className="bg-primary/10 border border-primary/20 rounded-3xl p-6 text-center">
                ...
                <a href="tel:5404186626" className="block">
                  <Button className="w-full font-semibold" size="sm"><Phone className="mr-2 h-3.5 w-3.5" /> 540-418-6626</Button>
                </a>
              </div>
            </motion.div>
          </div>
